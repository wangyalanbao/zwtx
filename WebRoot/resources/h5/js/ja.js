...
!function (window, document, screen, navigator) {
    ...
    function Store() {
        if ("undefined" == typeof Store._initialized) {
            var a = document.domain.replace(/.*?(\w+\.\w+)$/, "$1"), b = function (a) {
                ...
                window.localStorage.removeItem(a), window.localStorage.setItem(a, b)
            }, e = function (a) {
    ...
    "object" != typeof JSON && (JSON = {}), function () {
        ...
        "function" != typeof JSON.stringify && (escapable = /[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g, meta = {
            ...
            var c;
            if (gap = "", indent = "", "number" == typeof b)for (c = 0; b > c; c += 1)indent += " "; else"string" == typeof b && (indent = b);
            return str("", {"": a})
    ...
    var ja_code_version = "1.1", clstag_service_type_id = "cl", clstag_service_version = 1, heatmap_service_type_id = "hm", heatmap_service_version = 1, pv_service_type_id = "pv", pv_service_version = 1, _JdaExpireSeconds = 15552e3, _JdbExpireSeconds = 1800, _JdvExpireSeconds = 1296e3, _JduExpireSeconds = 15552e3, mtSubsiteExpireSeconds = 31536e3, __cookie_jda = jap.__cookie_jda || "__jda", __cookie_jdb = jap.__cookie_jdb || "__jdb", __cookie_jdc = jap.__cookie_jdc || "__jdc", __cookie_jdu = jap.__cookie_jdu || "__jdu", __cookie_jdv = jap.__cookie_jdv || "__jdv", _log_url_ = ("https:" == document.location.protocol ? "https://venus" : "http://venus") + ".jd.com/log.gif", _uuid_url_ = ("https:" == document.location.protocol ? "https://venus" : "http://venus") + ".jd.com/view-uuid";
    ...
    function hash(a) {
        var b = 1, c = 0, d;
        if (a) {
            for (b = 0, d = a.length - 1; d >= 0; d--)c = a.charCodeAt(d);
            b = (b << 6 & 268435455) + c + (c << 14), c = 266338304 & b, b = 0 !== c ? b ^ c >> 21 : b
        }
        return b
    }
    ...
    function join(a, b) {
        if (b || (b = "|||"), a instanceof Array) {
            for (var c = "", d = 0, e = a.length; e > d; d++)c += a[d] + (d == e - 1 ? "" : b);
            return c
        }
        return a
    }
    ...
    function Ic() {
        var a = Hc(), b = window.navigator, c = b.appName + b.version + a.language + b.platform + b.userAgent + a.javaEnabled + a.D + a.C + (document.cookie ? document.cookie : "") + (document.referrer ? document.referrer : ""), d = c.length, e = window.history.length;
        while (e > 0)c += e-- ^ d++;
        return hash(c)
    }
    ...
    var jsonp = function (a) {
        ...
        }), e = function (b, c, e, g, h) {
            var i = document.getElementsByTagName("head")[0], j = document.createElement("script");
            j.async = !0, h || (h = "utf-8");
            var k = "ja_cls_jsonp" + f.guid++, l = /^[0-9]+.?[0-9]*$/;
            g && l.test(g) || (g = 1e3 * f.request_timeout), finalUrl = b.replace("callback=?", "callback=" + k);
            var m = new Date, n = d.create({
                id: k,
                url: finalUrl,
                charset: h,
                failed: e,
        ...
        return b(f, {guid: 0, requests: {}, avail_tag: [], request_timeout: 5}), f
    }(window), siteId = jap.siteId || "JA2015_111145", account = "undefined" != typeof jap.account ? jap.account : store.getItem("pin"), pinId = store.getItem("pinId") || "", topic = jap.topic || "traffic-oth", adsCookieName = jap.adsCookieName || "", ads = "", pre_app = "", extParams = jap.extParams && "object" == typeof jap.extParams ? jap.extParams : "", domain = document.domain.replace(/.*?(\w+\.\w+)$/, "$1"), currentTime = Math.round((new Date).getTime() / 1e3), pageTitle = document.title, refererUrl = document.referrer, currentUrl = document.location.href, browserInfo = function () {
        var a = {name: "other", version: "0"}, b = navigator.userAgent.toLowerCase();
        browserRegExp = {
            se360: /360se/,
            se360_2x: /qihu/,
            ie: /msie[ ]([\w.]+)/,
        ...
        return a
    }(), browserName = browserInfo.name, browserVersion = browserInfo.version, os = function () {
    ...
    }(), screenResolution = screen ? screen.width + "x" + screen.height : "", colorDeep = screen ? screen.colorDepth + "-bit" : "", language = (navigator && (navigator.language || navigator.browserLanguage) || "").toLowerCase(), javaEnabled = navigator && navigator.javaEnabled() ? "1" : "0", charset = document.characterSet || document.charset || "", visitTimes = 1, sequenceNum = 0, jda = (store.getItem(__cookie_jda) || "").split("."), jdb = (store.getItem(__cookie_jdb) || "").split("."), jdv = (store.getItem(__cookie_jdv) || "").split("|"), jdu = store.getItem(__cookie_jdu) || "", jdc = store.getItem(__cookie_jdc) || "", firstSessionCreateTime, prevVisitCreateTime, uuid, all_uuid, hashDomain, utm_source = "direct", utm_campaign = "-", utm_medium = "none", utm_term = "-", flash_ver = "";
    ...
    var umz = [], isNewSession = jdb.length < 4, utsr = getParameter(currentUrl, "utm_source");
    if (utsr) {
        var camp = getParameter(currentUrl, "utm_campaign"), medi = getParameter(currentUrl, "utm_medium"), term = getParameter(currentUrl, "utm_term");
        umz.push(utsr || utm_source), umz.push(camp || utm_campaign), umz.push(medi || utm_medium), umz.push(term || utm_term), utm_term = umz[3]
    } else {
        var search = "baidu:wd,baidu:word,so.com:q,so.360.cn:q,360so.com:q,360sou.com:q,baidu:q1,m.baidu:word,m.baidu:w,wap.soso:key,m.so:q,page.yicha:key,sz.roboo:q,i.easou:q,wap.sogou:keyword,google:q,soso:w,sogou:query,youdao:q,ucweb:keyword,ucweb:word,114so:kw,yahoo:p,yahoo:q,live:q,msn:q,bing:q,aol:query,aol:q,daum:q,eniro:search_word,naver:query,pchome:q,images.google:q,lycos:query,ask:q,netscape:query,cnn:query,about:terms,mamma:q,voila:rdata,virgilio:qs,alice:qs,yandex:text,najdi:q,seznam:q,search:q,wp:szukaj,onet:qt,szukacz:q,yam:k,kvasir:q,ozu:q,terra:query,rambler:query".split(","), sign = !0, rdomain = refererUrl && refererUrl.split("/")[2];
    ...
    var preSession, preSeqnum, userAgent = navigator.userAgent;
    ...
    function log(a, b, c) {
        var d = "ot";
        ("pv" === a || "cl" === a) && (d = a);
        var e = JSON.stringify(c), f = _log_url_ + "?ut=s&d=web&lgt=" + encodeURIComponent(a) + "&lgv=" + encodeURIComponent(b) + "&sfj=" + encodeURIComponent(e) + "&uid=" + encodeURIComponent(jaCommonData.uuid) + "&pid=" + encodeURIComponent(jaCommonData.pinId) + "&cul=" + encodeURIComponent(jaCommonData.currentUrl) + "&ref=" + encodeURIComponent(jaCommonData.refererUrl) + "&pin=" + encodeURIComponent(jaCommonData.account) + "&ws=" + encodeURIComponent(jaCommonData.siteId) + "&vt=" + encodeURIComponent(jaCommonData.visitTimes) + "&sn=" + encodeURIComponent(jaCommonData.sequenceNum) + "&cut=" + encodeURIComponent(Math.round((new Date).getTime() / 1e3)) + "&tpc=" + encodeURIComponent(jaCommonData.topic + "." + d), g = new Image(1, 1);
    ...
    jaService.lg = function (a, b, c) {
        var d = store.getItem(jaCommonData.adsCookieName), e = {};
        if (ja_code_version && (e.jvr = ja_code_version), jaCommonData.utm_source && (e.usc = jaCommonData.utm_source), jaCommonData.utm_campaign && (e.ucp = jaCommonData.utm_campaign), jaCommonData.utm_medium && (e.umd = jaCommonData.utm_medium), jaCommonData.utm_term && (e.utr = jaCommonData.utm_term), jaCommonData.adsCookieName && (e.adk = jaCommonData.adsCookieName), d && (e.ads = d), c && "object" == typeof c)for (var f in c)e[f] = c[f];
        log(a, b, e)
    }, window.lg = jaService.lg, jaService.logHeatmap = function (a, b, c, d) {
        var e = store.getItem(jaCommonData.adsCookieName), f = {};
        ...
        var c = store.getItem(jaCommonData.adsCookieName), d = {};
        ...
        var a = store.getItem(jaCommonData.adsCookieName), b = 0;
    ...
    document.onclick = function (a) {
        a = a || event;
        var b = a.srcElement || a.target, c = b, d = b.getAttribute("clstag");
        while (!d && "BODY" != b.nodeName && "HTML" != b.nodeName) {
            if (b = b.parentNode, !b || "BODY" == b.nodeName && "HTML" != b.nodeName)break;
            d = b.getAttribute("clstag")
        }
        var e;
        if (d) {
            var f = d.split("|"), g = f[1], h = f[2], i = f[3];
            if ("keycount" === g) {
                e = h + "|||" + i;
                var j = c.getAttribute("href");
                jaService.logClstag(e, j), j && /http:\/\/.*?/.exec(j) && "_blank" !== c.getAttribute("target") && !isMeta(a) && (a.preventDefault ? a.preventDefault() : a.returnValue = !1, setTimeout(function () {
        ...
        if (jap.heatmapEnable && /(sale|mall|jmall|pop|bdp).(jd|360buy).com/.test(k)) {
            ...
            var n = window.screen.width >= 1210 ? 1210 : 990, o = document.body.clientWidth > n ? Math.round((document.body.clientWidth - n) / 2) : 0;
    ...
    if (isJdChannel() && isNotProcessByUnionSystem() && 1 === jaCommonData.sequenceNum) {
        var utm_source = jaCommonData.utm_source, utm_campaign = jaCommonData.utm_campaign, utm_medium = jaCommonData.utm_medium, utm_term = jaCommonData.utm_term, refererUrl = jaCommonData.refererUrl, rdomain = refererUrl && refererUrl.split("/")[2] || "", currentTime = jaCommonData.currentTime, channelId = 125, channelLevel = 3;
        "cpc" === utm_medium ? (channelLevel = 3, "baidu" === utm_source ? channelId = 60 : "sougou" === utm_source ? channelId = 61 : "google" === utm_source ? channelId = 62 : "youdao" === utm_source ? channelId = 63 : "360" === utm_source ? channelId = 64 : "media" === utm_source ? utm_term.startWith("media_12_") ? channelId = 60 : utm_term.startWith("media_13_") ? channelId = 61 : utm_term.startWith("media_14_") ? channelId = 64 : utm_term.startWith("media_4_") ? channelId = 65 : utm_term.startWith("media_10_") ? channelId = 66 : utm_term.startWith("media_11_") ? channelId = 126 : utm_term.startWith("media_8_") ? (channelLevel = 2, channelId = 67) : utm_term.startWith("media_5_") ? (channelLevel = 2, channelId = 68) : utm_term.startWith("media_9_") ? (channelLevel = 2, channelId = 69) : channelId = utm_term.startWith("media_15_") ? 144 : utm_term.startWith("media_16_") ? 144 : (utm_term.startWith("media_17_"), 144) : channelId = 144) : "ppc" === utm_medium ? (channelLevel = 3, channelId = 144) : "organic" === utm_medium ? (channelLevel = 3, channelId = "gouwu.sogou.com" === rdomain ? 187 : "gouwu.baidu.com" === rdomain ? 190 : "baidu" === utm_source ? 180 : "so.com" === utm_source ? 181 : "haoso.com" === utm_source ? 181 : "so.360.cn" === utm_source ? 181 : "sogou" === utm_source ? 182 : "google" === utm_source ? 183 : "bing" === utm_source ? 184 : 185) : "none" === utm_medium ? (channelLevel = 1, channelId = 122) : "referral" === utm_medium ? (channelLevel = 3, channelId = "gouwu.hao123.com" === rdomain ? 188 : "weigou.baidu.com" === rdomain ? 189 : 121) : "direct" === utm_source ? (channelLevel = 1, channelId = 122) : (channelLevel = 3, channelId = 125);
        var mt = store.getItem(jaCommonData.adsCookieName) || "", mtParts = mt.split("|");
        if (mtParts && 3 === mtParts.length)mtParts[channelLevel - 1] = encodeURIComponent(channelId + "," + currentTime); else {
            mtParts = [];
