(function (W) {
    ...
    function h() {
        ...
        function s(H, A, G, E) {
            ...
            if (!y[A.src]) {
                y[A.src] = {items: [], finished: false}
            }
            D = y[A.src].items;
            if (H[p] || D.length == 0) {
                F = D[D.length] = {ready: false, finished: false, ready_listeners: [C], finished_listeners: [B]};
                a(H, A, F, ((E) ? function () {
                    F.ready = true;
        ...
        function r() {
            ...
            function A() {
                if (!D || !D.scripts) {
                    F.push(D = {scripts: [], finished: true})
                }
            }
                    ...
                    for (var K = 0; K < arguments.length; K++) {
                        (function (M, O) {
                            ...
                            for (var N = 0; N < O.length; N++) {
                                ...
                                M = x(M, {ready: false, ready_cb: C, finished: false, finished_cb: B});
                                D.finished = false;
                                D.scripts.push(M);
                                s(G, M, D, (u && J));
                                J = true;
            ...
            return {
                script: H.script, wait: H.wait, setOptions: function (K) {
                    x(K, G);
                    return H
                }
            }
        ...
        o = {
            setGlobalDefaults: function (A) {
                ...
                z[z.length] = {type: "script", args: [].slice.call(arguments)};
                return o
            }, queueWait: function () {
                z[z.length] = {type: "wait", args: [].slice.call(arguments)};
                return o
            }, runQueue: function () {
...
(function (a) {
    ...
    BrowserUtil = function () {
        ...
        function p() {
            ...
            o = {}, m = {};
            var r = I.match(/Web[kK]it[\/]{0,1}([\d.]+)/), J = I.match(/(Android);?[\s\/]+([\d.]+)?/), K = !!I.match(/\(Macintosh\; Intel /), C = I.match(/(iPad).*OS\s([\d_]+)/), w = I.match(/(iPod)(.*OS\s([\d_]+))?/), u = !C && I.match(/(iPhone\sOS)\s([\d_]+)/), q = I.match(/(webOS|hpwOS)[\s\/]([\d.]+)/), E = q && I.match(/TouchPad/), v = I.match(/Kindle\/([\d.]+)/), H = I.match(/Silk\/([\d._]+)/), D = I.match(/(BlackBerry).*Version\/([\d.]+)/), A = I.match(/(BB10).*Version\/([\d.]+)/), s = I.match(/(RIM\sTablet\sOS)\s([\d.]+)/), B = I.match(/PlayBook/), G = I.match(/Chrome\/([\d.]+)/) || I.match(/CriOS\/([\d.]+)/), x = I.match(/Firefox\/([\d.]+)/), F = I.match(/MSIE\s([\d.]+)/) || I.match(/Trident\/[\d](?=[^\?]+).*rv:([0-9.].)/), t = !G && I.match(/(iPhone|iPod|iPad).*AppleWebKit(?!.*Safari)/), z = t || I.match(/Version\/([\d.]+)([^S](Safari)|[^M]*(Mobile)[^S]*(Safari))/), y = I.indexOf("MicroMessenger") >= 0;
            if (r) {
                m.browser = "webkit";
                m.version = r[1]
            }
    ...
    LocalStorageUtil = function () {
        ...
        return {get: m, set: q, remove: l, removeAll: n}
    };
    f.modules = {};
    f.runMod = [];
    f.config = {debug: 0};
    ...
    var e = {
        require: function (l, m) {
            ...
            $LAB.setOptions({AlwaysPreserveOrder: true}).script(l).wait(function () {
                if (m) {
                    m.call(null)
                }
            })
