M.define("index", function (a) {
    ...
    M.object.merge(b.prototype, {
        init: function (e) {
            this.threadTime = 500;
            this.thread = new (M.exports("thread").clazz)({time: this.threadTime});
            this.sliderTouchXY = {};
            this.sliderTime = 0;
            this.sliderImgIndex = 1;
            this.themeIdList = [];
            this.themeImgTimer = {};
            this.imgHigth = {all: 0, brand: {}, discount: {}};
            if (M.object.isObject(e)) {
                ...
                this.hasAppBanner = e && e.appBanner ? e.appBanner.isOpen : false;
                this.appBannerType = e && e.appBanner ? e.appBanner.indexClientType : 0;
                if (this.appBannerType == 1 && this.hasAppBanner) {
                    $("#index_search_main").css("top", "50px")
                }
            }
            ...
            c.sliderHTML();
            c.sliderSetLiWidth();
            c.thread.addOne({name: "slider", event: c.sliderRun, source: c});
            ...
            $("#slider").on("touchmove", function (g) {
                var j = g.touches;
            ...
            var c = this;
            c.thread.addOne({name: "theme", event: c.themeImgRun, source: c})
            ...
            var c = this;
            c.thread.addOne({name: "seckill", event: c.seckillRun, source: c})
        }, appBannerBind: function () {
            var i = this;
            if (i.hasAppBanner) {
                $("#index_banner").on("touchmove", function (l) {
                ...
                $("#index_openApp").on("touchmove", function (l) {
                    if (l && l.preventDefault) {
                        l.preventDefault()
                    } else {
                        window.event.returnValue = false;
                ...
                $("#index_openWeb").on("touchmove", function (l) {
                    if (l && l.preventDefault) {
                        l.preventDefault()
                    } else {
                        window.event.returnValue = false;
                ...
                var f = navigator.userAgent.indexOf("360 Aphone") >= 0 || navigator.userAgent.indexOf("QHBrowser") >= 0 || navigator.userAgent.indexOf("360 Iphone") >= 0;
                var k = M.url.getParam("utm_campaign");
                if (k && k == "t_vivobrowser_vivobrowser") {
                    f = true
                }
                if (!f) {
                    var c = null;
                    var e = null;
                    var h = "http://h5.m.jd.com/active/download/download.html?channel=jd-mxz3";
                    var j = "MDownLoadFloat_Expose_Virtual";
                    if (i.appBannerType == 1) {
                        c = "MDownLoadFloatTop_OpenNow";
                        e = "MDownLoadFloatTop_Close";
                        j = "MDownLoadFloatTop_Expose_Virtual";
                        h = "http://h5.m.jd.com/active/download/download.html?channel=jd-mxz1"
                    } else {
                        if (i.appBannerType == 2) {
                            c = "MDownLoadFloatBottom_OpenNow";
                            e = "MDownLoadFloatBottom_Close";
                            j = "MDownLoadFloatBottom_Expose_Virtual";
                            h = "http://h5.m.jd.com/active/download/download.html?channel=jd-mxz2"
                        }
                    }
                    var g = {
                        openAppBtnId: "index_openApp",
                        closePanelBtnId: "index_openWeb",
                        closePanelId: "index_banner",
                        downAppURl: h,
                        closeCallblack: function () {
                            var l = this;
                            $("#index_banner").attr("search_land_searchTransformation_show", "true");
                            if (l.appBannerType == 0) {
                                l.fireworksLaod()
                            }
                            l.indexFloatLayerRunOpen = true;
                            if (l.appBannerType == 1) {
                                $("#index_search_main").css("top", "0px")
                            }
                        },
                        closeCallblackSource: i,
                        cookieFlag: "m_index",
                        openAppEventId: c,
                        closePanelEventId: e
                    };
                    ...
                    if ($("#index_banner").css("display") != "none") {
                        M.require("http://h5.m.jd.com/active/track/mping.min.js", function () {
                            try {
                                var m = new MPing.inputs.Click(j);
                                m.event_param = "";
                                var l = new MPing();
                                l.send(m)
                            } catch (n) {
            ...
            if (c.floatUrl) {
                c.thread.addOne({
                    name: "floatLayer", event: function () {
                        var f = this;
            ...
            if (k && k.recommendList && k.recommendList.length > 0) {
                ...
                var i = "http://m.jd.com?recommend=1";
            ...
            if (g && g.recommendList && g.recommendList.length > 0) {
                for (var c = 0; c < g.recommendList.length; c++) {
                    ...
                    e.push('<div alt="' + g.recommendList[c].wname + '" class="love-item-pic"><img src="/images/index/m-home-pic.png" imgsrc="' + g.recommendList[c].imageurl + '" ></div>');
            ...
            var c = this;
            if (c.recommendOpen) {
                $("#" + c.recommendListLoadMoreBtn).on("click", function () {
                    c.recommendCanLoad = true;
                    c.recommendLoadData()
                })
            }
    ...
    M.object.merge(b.prototype, {
        init: function (e) {
            ...
            if (c == "" && e.searchLoop) {
                ...
                $("#" + e.claerKeywordBtnId).css({display: "none"})
            } else {
                if (c != "" && e.oldkeyword != c && e.searchLoop) {
                    e.searchLoadFromKeyword(c);
                    $("#" + e.claerKeywordBtnId).css({display: "block"})
                }
            }
            setTimeout(function () {
                e.searchLoadControl()
            ...
            var g = this;
            g.searchLoop = false;
            if (!g.searchLocalStorage) {
                var c = M.localstorage.get(g.searchHistoryLocalStorageName);
                try {
                    c = JSON.parse(c)
                } catch (h) {
                    c = null;
                    M.localstorage.remove(g.searchHistoryLocalStorageName)
                }
                if (c != null && !M.object.isArray(c)) {
                    c = null;
                    M.localstorage.remove(g.searchHistoryLocalStorageName)
                }
                if (c && c.length > 0 && g.openSearchLoad) {
                    var f = c;
                    g.searchLandUnbind();
                    $("#" + g.controlId).html(g.searchLandLiHtml(f));
                    $("#" + g.clearHistoryId).parent().css({display: "block;"});
                    g.searchLandBind(true);
                    g.tipArray = f;
                    $("#" + g.controlId).removeClass("jd-auto-complete");
                    $("#" + g.controlId).children("li").css({display: "block"});
                    g.searchLoop = true
                } else {
                    $("#" + g.controlId).html("");
                    g.searchLoop = true
            ...
            M.http.ajax({
                url: "/ware/searchSuggestion.action", data: {keyword: c, _format_: "json"}, success: function (g) {
                    if (g && g.searchTipList && e.openSearchLoad) {
                        var f = JSON.parse(g.searchTipList);
                        if (f.length > 0) {
                            e.searchLandUnbind();
                            $("#" + e.hotKeyWordId).hide();
                            ...
                            e.tipArray = f;
                            $("#" + e.clearHistoryId).parent().css({display: "none;"});
            ...
            var f = this;
            if (c < e) {
                $("#" + f.searchliIdList[c]).fadeIn(10, function () {
                    var g = c + 1;
            ...
            if (f.searchliIdList && f.searchliIdList.length > 0) {
                for (var e = 0, c = f.searchliIdList.length; e < c; e++) {
            ...
            if (g.searchliIdList.length > 0) {
                ...
                for (var e = 0; e < g.searchliIdList.length; e++) {
                    $("#" + g.searchliIdList[e]).on("click", function () {
                        var h = $(this).attr("searchland_index");
                        var i = g.tipArray[h];
                        g.searchLandSubmit(i, (f ? "MSearch_HistoryRecords" : "MSearch_SearchDropDownAssociationalWords"))
                    })
                }
            }
            ...
            if (M.object.isObject(c)) {
                h = c;
                f = JSON.parse(h.value)
            } else {
                h = {};
                f = {};
                f.keyword = c;
                h.value = JSON.stringify(f);
                h.text = c
            }
            g.sendMping(e, f.keyword);
            $("#" + g.keywordId).val(f.keyword);
            f.catelogyList && $("#" + g.catelogyListId).val(JSON.stringify(f.catelogyList));
            g.searchLandAddHistory(h);
            if (h.otherAttr && h.otherAttr.isSearchShop) {
                $("#" + g.formId).attr("action", "/shop/searchShop.action")
            } else {
                $("#" + g.formId).attr("action", "/ware/search.action")
            }
            $("#" + g.formId).submit();
            g.searchLoop = false
        }, searchLandAddHistory: function (l) {
            var j = this;
            var g = M.localstorage.get(j.searchHistoryLocalStorageName);
            try {
                g = JSON.parse(g)
            } catch (k) {
                g = null;
                M.localstorage.remove(j.searchHistoryLocalStorageName)
            }
            if (!M.object.isArray(g)) {
                g = null;
                M.localstorage.remove(j.searchHistoryLocalStorageName)
            }
            l.text = j.makeSearchName(l.text);
            delete l.tipNumber;
            if (g && g.length > 0) {
                var h = 0;
                var c = [l];
            ...
            M.localstorage.set(j.searchHistoryLocalStorageName, JSON.stringify(g))
            ...
            e.push("</div>");
            return e.join("")
        }, sendMping: function (g, c) {
            try {
                var h = new MPing.inputs.Click(g);
                h.event_param = c ? c : "";
                var f = new MPing();
                f.send(h)
            } catch (i) {
            ...
            $("body").addClass("hide-landing");
            $("#" + e.keywordId).on("click", function (h) {
                var g = null;
                if (e.isHome) {
                    if (e.openSearchLoad) {
                        g = "MSearch_Search"
            ...
            $("#" + e.keywordId).on("keydown", function (h) {
                if (h.keyCode == "13") {
                    var g = $.trim($("#" + e.keywordId).val());
                    if (g != "") {
                        e.searchLandSubmit(g, "MSearch_Searchthi")
                    }
                }
                return
            });
            $("#" + e.closeSearchLandBtnId).on("click", function () {
                if (e.openSearchLoad) {
                    e.searchTransformation();
                    $("#" + e.keywordId).val(e.keyword);
                    $("#" + this.keywordId).addClass("hilight1")
            ...
            $("#" + e.clearHistoryId).on("click", function () {
                e.sendMping("MSearch_ClearHistory");
                e.clearHistory();
                $("#" + e.controlId).html("");
                $("#" + e.clearHistoryId).parent().css({display: "none;"})
            });
            $("#" + e.submitId).on("click", function () {
                ...
                if (h != "") {
                    ...
                    e.searchLandSubmit(h, g)
                }
            });
            $("#" + e.claerKeywordBtnId).on("click", function () {
                $("#" + e.keywordId).val("");
                $(this).css({display: "none;"})
            });
            $("#" + e.keywordId).on("keyup", function () {
            ...
            if (e.hotKeyWordBtnIds.length > 0) {
                for (var c = 0; c < e.hotKeyWordBtnIds.length; c++) {
                    $("#" + e.hotKeyWordBtnIds[c]).on("click", function () {
                        var g = $.trim($(this).find("span").text());
                        if (g != "") {
                            e.searchLandSubmit(g, "MSearch_HotWords")
                        }
                    })
                }
            }
            if (e.hotKeyword && e.hotKeyword.length > 0) {
                $("#" + e.changeHotKeyWordId).on("click", function () {
                    var h = 0;
                    var l = [];
                    for (var k = e.hotKeywordIndex + 1, g = k + 6; k < g && k < e.hotKeyword.length; k++) {
                        h++;
                        e.hotKeywordIndex = k;
                        l.push(e.hotKeyword[k])
                    }
                    if (h < 6) {
                        for (var k = 0, g = 6 - h; k < g && k < e.hotKeyword.length; k++) {
                            h++;
                            e.hotKeywordIndex = k;
                            l.push(e.hotKeyword[k])
                        }
