...
<%@page import="com.hxlm.health.web.CommonAttributes"%>
<%@page import="com.hxlm.health.web.util.JsonUtils"%>
...
<%
	...
	if (status.equals("success")) {
		...
		try {
			Resource resource = new ClassPathResource(CommonAttributes.HEALTHLM_PROPERTIES_PATH);
			...
			status = "error";
			message = "HEALTHLM.PROPERTIES文件写入失败!";
			exception = stackToString(e);
		}
	}
	...
	if (status.equals("success")) {
		try {
			...
			Element siteUrlElement = (Element) document.selectSingleNode("/healthlm/setting[@name='siteUrl']");
			Element logoElement = (Element) document.selectSingleNode("/healthlm/setting[@name='logo']");
			Element defaultLargeProductImageElement = (Element) document.selectSingleNode("/healthlm/setting[@name='defaultLargeProductImage']");
			Element defaultMediumProductImageElement = (Element) document.selectSingleNode("/healthlm/setting[@name='defaultMediumProductImage']");
			Element defaultThumbnailProductImageElement = (Element) document.selectSingleNode("/healthlm/setting[@name='defaultThumbnailProductImage']");
			String siteUrl;
			...
			try {
				...
				fileOutputStream = new FileOutputStream(healthlmXmlFile);
	...
	if (status.equals("success")) {
		try {
			FileUtils.writeStringToFile(new File(rootPath + INSTALL_LOCK_CONFIG_PATH), "HEALTHLM INSTALL LOCK");
		} catch (IOException e) {
			status = "error";
	...
	if (status.equals("success")) {
		try {
			FileUtils.writeStringToFile(new File(rootPath + INSTALL_INIT_CONFIG_PATH), "HEALTHLM INSTALL INIT");
