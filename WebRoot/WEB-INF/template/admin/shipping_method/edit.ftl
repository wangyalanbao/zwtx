...
$().ready(function() {
    ...
    // 地区选择
    $areaId.lSelect({
        url: "${base}/admin/common/areas.jhtml"
    });
...
<body>
	...
	<form id="inputForm" action="update.jhtml" method="post">
		...
		<table class="input">
            ...
            <tr>
                <th>
                    <span class="requiredField">*</span>${message("ShippingMethod.sendArea")}:
                </th>
                <td>
                    <input type="text" name="sendArea" class="text"  value="${sendArea.name}" readonly title="默认地址不可修改" />
                </td>
            </tr>
            <tr>
                <th>
				${message("ShippingMethod.receiveArea")}:
                </th>
                <td>
               <span class="fieldSet">
                   <input type="hidden" id="areaId" name="inputArea" value="${(shippingMethod.receiveArea.id)!}" treePath="${(shippingMethod.receiveArea.treePath)!}"/>
               </span>
                </td>
            </tr>
			<tr>
