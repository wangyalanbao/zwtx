...
public class Cryptos {
	...
	//--快速解密算法，输入key和byte[]，缺省长度为byte[]长度--//
	public static String crypto_quick_dec_ByteArr_Tx(vm_crypto_quick_key_t key, byte[] src, int len) {
	...
	public static byte[] vm_crypto_quick_enc_ByteArr(vm_crypto_quick_key_t key, String srcTx) {
		//处理输入值 ，txt->byte[]->int[]
		byte[] bt = srcTx.getBytes();
	...
	************************************************************************/
	 ...
	 * @return
	 */
	public static String toDecodeQES(String key, String srcTx){
		vm_crypto_quick_key_t keyt = vm_crypto_quick_init_key_Str(key);//初始化key
		byte [] byteTx = b64Decode(srcTx);//base64解码
		String bty = crypto_quick_dec_ByteArr_Tx(keyt, byteTx, byteTx.length);//解密
		return bty;
	}
	...
	//--base64编码--//  
	public static String b64Encode(byte[] bstr) {
	...
	public static void main(String[] args) {
		System.out.println("460403573292626");
		System.out.println("460303573292626");
		System.out.println("460203573292621");
		System.out.println("460103573292622");
	}
