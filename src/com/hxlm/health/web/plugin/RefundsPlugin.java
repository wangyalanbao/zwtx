 ...
 * 
 */
package com.hxlm.health.web.plugin;
 ...
 * 
 */
public abstract class RefundsPlugin implements Comparable<RefundsPlugin> {

	/** 支付方式名称属性名称 */
	public static final String REFUNDS_NAME_ATTRIBUTE_NAME = "refundsName";
	...
	public String getRefundsName() {
		PluginConfig pluginConfig = getPluginConfig();
		return pluginConfig != null ? pluginConfig.getAttribute(REFUNDS_NAME_ATTRIBUTE_NAME) : null;
	}
	 ...
	 * @return 请求参数
	 */
	public abstract String  getParameterXml(String sn,  HttpServletRequest request, HttpServletResponse response);


	public abstract SecapiPayRefund getSecapiPayRefund(String sn, HttpServletRequest request, HttpServletResponse response);
	/**
	 * 验证通知是否合法
	 * @param refundsMap
	 * @param request
	 ...
	 * @return
	 */
	public abstract boolean verifyNotifyWeiXin(Map refundsMap, HttpServletRequest request, HttpServletResponse response);
	 ...
	 * @param sn
	 *            编号(忽略大小写)
	 * @return 退款单，若不存在则返回null
	 */
	protected Refunds getRefunds(String sn) {
		return refundsService.findBySn(sn);
	}
	...
	protected String getNotifyUrl(String sn, NotifyMethod notifyMethod) {
		...
		if (notifyMethod == null) {
			return setting.getSiteUrl() + "/admin/refunds/notify/" + NotifyMethod.general + "/" + sn + ".jhtml";
		}
		return setting.getSiteUrl() + "/admin/refunds/notify/" + notifyMethod + "/" + sn + ".jhtml";
	}
	...
	public boolean equals(Object obj) {
		...
		RefundsPlugin other = (RefundsPlugin) obj;
	...
	public int compareTo(RefundsPlugin refundsPlugin) {
		return new CompareToBuilder().append(getOrder(), refundsPlugin.getOrder()).append(getId(), refundsPlugin.getId()).toComparison();
	}
