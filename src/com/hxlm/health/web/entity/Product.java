...
import com.hxlm.health.web.BigDecimalNumericFieldBridge;
...
@SequenceGenerator(name = "sequenceGenerator", sequenceName = "lm_product_sequence")
public class Product extends BaseEntity {
	...
	/** 静态路径 */
	private static String mobileStaticPath;

	/** 静态路径 */
	private static String padStaticPath;
	...
	static {
		try {
			...
			org.dom4j.Document document = new SAXReader().read(healthlmXmlFile);
			org.dom4j.Element element = (org.dom4j.Element) document.selectSingleNode("/healthlm/template[@id='productContent']");
			staticPath = element.attributeValue("staticPath");

			org.dom4j.Element mobileElement = (org.dom4j.Element) document.selectSingleNode("/healthlm/template[@id='mobileProductDetail']");
			mobileStaticPath = mobileElement.attributeValue("staticPath");
			org.dom4j.Element padElement = (org.dom4j.Element) document.selectSingleNode("/healthlm/template[@id='padProductDetail']");
			padStaticPath = padElement.attributeValue("staticPath");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	@Valid
	@ElementCollection
	@CollectionTable(name = "lm_product_product_image")
	...
	@CollectionTable(name = "lm_product_member_price")
	 ...
	 */
	@ElementCollection(fetch = FetchType.LAZY)
	@CollectionTable(name = "lm_product_parameter_value")
	...
	public String getMobilePath() {
		...
		model.put("productCategory", getProductCategory());
		try {
			return FreemarkerUtils.process(mobileStaticPath, model);
		} catch (IOException e) {
			e.printStackTrace();
		} catch (TemplateException e) {
			e.printStackTrace();
	...
	public String getPadPath() {
		...
		model.put("productCategory", getProductCategory());
		try {
			return FreemarkerUtils.process(padStaticPath, model);
		} catch (IOException e) {
			e.printStackTrace();
		} catch (TemplateException e) {
			e.printStackTrace();
