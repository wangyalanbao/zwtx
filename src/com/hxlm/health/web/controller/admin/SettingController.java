...
import com.hxlm.health.web.Setting.ConsultationAuthority;
import com.hxlm.health.web.Setting.ReviewAuthority;
import com.hxlm.health.web.Setting.RoundType;
import com.hxlm.health.web.Setting.StockAllocationTime;
import com.hxlm.health.web.Setting.WatermarkPosition;
import com.hxlm.health.web.service.CacheService;
...
public class SettingController extends BaseController {
	...
	public String update(Setting setting, MultipartFile watermarkImageFile, RedirectAttributes redirectAttributes) {
		...
		try {
			org.springframework.core.io.Resource resource = new ClassPathResource(CommonAttributes.HEALTHLM_PROPERTIES_PATH);
			...
			if (setting.getIsDevelopmentEnabled()) {
				if (!templateUpdateDelay.equals("0") || !messageCacheSeconds.equals("0")) {
					...
					properties.store(outputStream, "HEALTHLM PROPERTIES");
				}
			} else {
				if (templateUpdateDelay.equals("0") || messageCacheSeconds.equals("0")) {
					outputStream = new FileOutputStream(resource.getFile());
