				...
				for (int i = 0; i < properties.length; i++) {
					...
					if (converters != null && converters.length > i && converters[i] != null) {
						...
						if(properties[i].equals("%orderItems%")){ // 属性为订单项集合时
							...
							List<OrderItem> orderItemList  = ((com.hxlm.health.web.entity.Order)item).getOrderItems();
							for(OrderItem orderItem:orderItemList){
								stringBuilder = stringBuilder.append(orderItem.getFullName() + " " + orderItem.getQuantity() + "件");
								stringBuilder = stringBuilder.append("  |  ");
							}
							cell.setCellValue(stringBuilder.toString());
						}else if(properties[i].equals("%payments%")) { // 属性为付款单集合时
							Set<Payment> paymentSet = ((com.hxlm.health.web.entity.Order) item).getPayments();
							Date date = null;
							Iterator<Payment> iterator = paymentSet.iterator();
							while (iterator.hasNext()) {
								Payment payment = iterator.next();
								if (date == null) {
									date = payment.getModifyDate();
								} else {
									if (payment.getModifyDate().after(date)) {
										date = payment.getModifyDate();
									}
								}
							}
						...
						}else if(properties[i].equals("%orderPassengers%")) { // 乘客属性集合时
							StringBuilder stringBuilder = new StringBuilder();
							List<OrderPassenger> orderPassengerList = ((OrderAirline) item).getOrderPassengers();
							for (OrderPassenger passenger:orderPassengerList){
								stringBuilder = stringBuilder.append(passenger.getName() + ",证件号：" +passenger.getIdCardNo());
								stringBuilder = stringBuilder.append("  |  ");
							}
							cell.setCellValue(stringBuilder.toString());
						}
