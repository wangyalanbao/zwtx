...
import com.hxlm.health.web.entity.CartItem;
...
public class ProductCategoryServiceImpl extends BaseServiceImpl<ProductCategory, Long> implements ProductCategoryService {
	...
	@Transactional(readOnly = true)
	public ProductCategory findRoot(Product product,Cart cart) {
		...
		if(product != null){
			...
			for (CartItem cartItem : cart.getCartItems()) {
				if (cartItem != null && cartItem.getProduct() != null) {
					productCategory = cartItem.getProduct() == null?null:cartItem.getProduct().getProductCategory();
					break;
				}
			}
		}
		if(productCategory !=null){
			...
			List<ProductCategory> productCategoryList = this.findParents(productCategory);
	...
	@Transactional(readOnly = true)
	public List<ProductCategory>  findListByName(String name) {
		return productCategoryDao.findListByName(name);
	}
	...
	public Long saveProductCategory(ProductCategory productCategory) {
		super.save(productCategory);
		return productCategoryDao.getIdentifier(productCategory);
	}
