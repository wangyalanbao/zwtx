...
import com.hxlm.health.web.entity.AirplaneImage;
import com.hxlm.health.web.plugin.StoragePlugin;
import com.hxlm.health.web.service.AirplaneImageService;
 ...
 * 
 */
@Service("airplaneImageServiceImpl")
public class AirplaneImageServiceImpl implements AirplaneImageService, ServletContextAware {
	...
	public void build(AirplaneImage airplaneImage) {
		MultipartFile multipartFile = airplaneImage.getFile();
			...
			try {
				...
				for (StoragePlugin storagePlugin : storagePlugins) {
					if (storagePlugin.getIsEnabled()) {
						...
						airplaneImage.setSource(storagePlugin.getUrl(sourcePath));
						airplaneImage.setLarge(storagePlugin.getUrl(largePath));
						airplaneImage.setMedium(storagePlugin.getUrl(mediumPath));
						airplaneImage.setThumbnail(storagePlugin.getUrl(thumbnailPath));
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
